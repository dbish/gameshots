...
function addNewPost(info){
        ...
        var mediaHolder = $('<div class="mediaHolder">').appendTo(post);
        if (info[2].includes('.twitch.tv') || info[2].includes('.youtube.com')){
                mediaHolder.append('<iframe src="'+info[2]+'" frameborder="0" scrolling="no" alowwfullscreen="true"></iframe>');
        }
        else if (info[2].includes('.mp4')){
                mediaHolder.append('<video controls muted autoplay class="card-img-top"><source src="'+info[2]+'" type="video/mp4"></video>');
        }else{
                mediaHolder.append('<img src="'+info[2]+'" class="card-img-top">');
        }
